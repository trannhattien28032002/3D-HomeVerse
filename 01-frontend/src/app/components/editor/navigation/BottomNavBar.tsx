/**
 * BottomNavBar — thanh công cụ nổi ở giữa-dưới màn hình editor.
 *
 * Hiển thị các nút theo `mode` hiện tại (lọc từ BOTTOM_NAV theo modes). Mỗi nút có
 * thể: đổi chế độ 2D/3D, chọn tool 2D (select/draw), đổi gizmo (translate/rotate),
...
export default function BottomNavBar({ mode, activeNav, setMode, setToolMode2D, engine, onDecorClick, onColorClick, gizmoMode = "translate", onGizmoModeChange }: Props) {
    ...
    return (
        <div style={{
            ...
            {visibleItems.map((item) => {
                ...
                // Nút gizmo (chỉ hiện ở 3D) highlight theo gizmoMode; activeNav không
                // bao giờ nhận "translate"/"rotate" (chỉ select/build/decor).
                const isActive = (item.id === "translate" || item.id === "rotate")
                    ? gizmoMode === item.id
                    : activeNav === item.id;
                return (
                    <button
