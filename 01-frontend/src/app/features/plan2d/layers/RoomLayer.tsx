...
import type { Room2D } from "src/app/features/plan2d/types";
        ...
        <Layer listening={activeTool2D === "select"}>
            ...
            {rooms.map(room => (
                <Line key={room.id} points={room.polygon.flatMap(p => [p.x, p.y])} closed
                    fill={alpha(RGB.primaryContainer, 0.10)} stroke={alpha(RGB.primary, 0.18)} strokeWidth={2} lineJoin="round"
                    ...
                    return (
                        <Group key={`area-${room.id}`} listening={false}>
                            <Rect
                                ...
                                stroke={alpha(RGB.primary, 0.22)} strokeWidth={sw}
                                cornerRadius={cr}
                            />
                            <Text
                                x={room.centroidX} y={room.centroidY}
