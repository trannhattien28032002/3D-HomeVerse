 ...
 *
 * Sống ở `shared/` (không phải `engine/serialization/`) vì đây là contract lưu trữ
 * mà `data/` (scenesApi, versionsApi) cần biết type mà không được phép phụ thuộc
 * `engine` — xem BAO-CAO-REVIEW-DA-CHIEU.md #3. `engine/serialization` vẫn là nơi
 * DUY NHẤT hiện thực serialize/deserialize; type chỉ là contract dữ liệu thuần,
 * re-export lại qua `engine/serialization` (barrel) để code cũ không phải đổi import.
 */
...
export type SceneDocument = {
     ...
     * Optional — phòng không có uuid bền nên dùng roomKey. File cũ không có. Cho phép
     * phân biệt phòng (khách/bếp/ngủ/WC) sau khi generate. Key mồ côi nếu topology đổi
     * (chấp nhận; dọn sau ở milestone reconcile).
     */
