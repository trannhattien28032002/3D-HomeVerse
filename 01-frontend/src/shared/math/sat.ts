/**
 * SAT (Separating Axis Theorem) primitives — pure 2D geometry, framework-free.
 *
 * Đơn vị input/output là vô-đơn-vị: caller có thể truyền pixel (collision2D ở UI)
 * hoặc metres (engine layer). Mọi hàm trong file đều thuần — không state.
 *
 * SAT chỉ đúng với polygon LỒI (convex). Caller phải đảm bảo input là convex.
 * OBB (rotated rectangle) → 4 đỉnh là convex; miter-quad của tường cũng convex.
 *
 * Layer: shared/math — có thể import từ engine/ lẫn app/.
 *
 * Moved from src/app/components/editor/tools/collision2D.ts in Đợt 2.
 */
