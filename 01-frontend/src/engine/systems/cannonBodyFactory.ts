...
/** Resize probe body shape to match half-extents (with shrink). */
...
/** Create a new static CANNON.Body and matching StaticEntry from ECS components. */
...
/**
 * Sync an existing static body when transform/size may have changed.
 * Returns true if the body was actually updated.
 */
...
/** Remove stale static entries from both the Map and the physics world. */
export function gcStaticBodies(
    entries: Map<number, StaticEntry>,
    alive: number[],
    physicsWorld: CANNON.World,
): void {
...
/** Remove stale dynamic entries from the Map. */
export function gcDynamicEntries(entries: Map<number, DynamicEntry>, alive: number[]): void {
    const aliveSet = new Set(alive);
...
/**
 * Sweep CCD from `from` toward `to`, returning the last non-overlapping position.
 * `testFn` receives absolute (x, y, z) and returns true when a collision is detected.
 */
